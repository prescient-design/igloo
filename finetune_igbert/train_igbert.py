...
def main():
    ...
    if args.resume_from_checkpoint:
        ...
        if os.path.exists(os.path.join(args.resume_from_checkpoint, "epoch.txt")):
            with open(os.path.join(args.resume_from_checkpoint, "epoch.txt"), "r") as f:
                starting_epoch = int(f.read().strip()) + 1
                resume_step = None
                completed_steps = starting_epoch * num_update_steps_per_epoch
        elif os.path.exists(os.path.join(args.resume_from_checkpoint, "step.txt")):
    ...
    for epoch in range(starting_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(active_dataloader):
            with accelerator.accumulate(model):
                ...
                #     predicted_token_ids = torch.argmax(masked_logits, dim=-1)
                #     masked_true_labels = batch["labels"][masked_token_indices]
                #     aa_recovery = (predicted_token_ids == masked_true_labels).sum().item() / masked_true_labels.numel()
